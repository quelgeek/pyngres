...
##
##  Command syntax:
##      python apiscupd.py [vnode::]dbname[/server_class]
...
def IIdemo_init():
    ...
    print('IIdemo_init: calling IIapi_initialize()...')
...
prepText = b'PREPARE s0 FROM ' b'INSERT INTO api_demo_cupd ' b'VALUES( ?, ? )'

execText = b'EXECUTE s0'

openText = b'SELECT * ' b'FROM api_demo_cupd ' b'FOR UPDATE'

updateText = b'UPDATE api_demo_cupd ' b'SET age = -1'
...
while True:
    ...
    ##  update using cursor
    print(f'{script}: update row')
    qyp.qy_genParm.gp_callback = None
    qyp.qy_genParm.gp_closure = None
    qyp.qy_connHandle = connHandle
    qyp.qy_queryType = IIAPI_QT_CURSOR_UPDATE
    qyp.qy_queryText = updateText
    qyp.qy_parameters = True
    qyp.qy_tranHandle = tranHandle
    qyp.qy_stmtHandle = None
